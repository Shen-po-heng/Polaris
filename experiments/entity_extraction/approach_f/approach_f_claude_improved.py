"""
Approach F: Claude Haiku 4.5 + prefill + few-shot + CB acc improvements.

Improvements over Approach E:
1. Explicit "no pronoun" rule — ban "our model/network/method"; require actual name
2. Paper title injected as context — tells model what the main contribution is
3. (Evaluated with fuzzy CB acc in benchmark)

Usage:
    python experiments/entity_extraction/approach_f/approach_f_claude_improved.py
...
Output:
{"entities": [{"name": "Transformer", "type": "Contribution"}, {"name": "ByteNet", "type": "Baseline"}, \
...
"relations": [{"head": "Transformer", "tail": "ByteNet", "type": "outperforms"}, \
{"head": "Transformer", "tail": "ConvS2S", "type": "outperforms"}, \
{"head": "Transformer", "tail": "WMT14", "type": "evaluated_on"}]}
...
The paper being analyzed is titled: "{paper_title}"
This paper's main contribution is the novel method/system it proposes — label it as Contribution.

Given this text from the **{section_type}** section, extract entities and relations as JSON.
...
- NEVER use pronouns: do NOT write "our model", "our network", "our method", "our approach", \
"the proposed method" — always use the actual published name (e.g. "ResNet", "BERT", "AlexNet")
